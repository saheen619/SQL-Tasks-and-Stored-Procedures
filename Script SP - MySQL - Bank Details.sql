...
-- Making the jobrole dynamic where the user has to pass the jobrole
DELIMITER && 
CREATE PROCEDURE AVG_BAL_JOBROLE1 (IN VAR varchar(30))
BEGIN 
  SELECT AVG(balance) FROM bank_details WHERE job = lower(VAR);
END &&
...
-- Using multiple values to be passed
