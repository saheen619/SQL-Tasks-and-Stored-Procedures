...
-- A hardcoded stored procedure
CREATE PROCEDURE area_calc_procedure()
  RETURNS VARCHAR
  LANGUAGE SQL
  AS
  $$
...
CALL area_calc_procedure();


CREATE OR REPLACE PROCEDURE myprocedure_parameter(inp_radius NUMBER(8,3))
  RETURNS VARCHAR
  LANGUAGE SQL
  AS
  $$
    DECLARE
      ...
      area_of_circle FLOAT;
    BEGIN
      radius_of_circle := inp_radius;                               --Variable Declaration in SQL
...
CALL myprocedure_parameter(8.76);


SHOW PROCEDURES; 


-- Just a dummy syntax to show how we can run stored procedure using python
CREATE OR REPLACE PROCEDURE myprocedure(from_table string, to_table string, COUNT int)
  RETURNS string
  LANGUAGE python
  RUNTIME_VERSIONS = '3.8'
...
-- While reading INPUT PARAMTERS use =: VAR_NAME
-- While assigning VARIABLES use := 
...
CALL GET_AVG_BALANCE('technician');
